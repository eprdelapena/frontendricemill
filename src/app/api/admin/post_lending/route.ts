...
import { TParamsPostLending, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsPostLending = await req.json();

    const response = await client.mainPostLending(body, config);
